...
import com.cafe.main.commons.utils.MediaUtils;
import com.cafe.main.commons.utils.UploadFileUtils;
import com.cafe.main.service.UploadService;
...
public class UploadController {
    ...
    // 게시글 첨부파일 조회
    @RequestMapping(value = "/list/{cno}")
    public ResponseEntity<List<String>> fileList(@PathVariable("cno") int cno) throws Exception {
        ResponseEntity<List<String>> entity = null;
        try {
            entity = new ResponseEntity<>(uploadService.getAttach(cno), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<String> boardMofifyRemoveFile(@PathVariable("cno") int cno, String fileName) throws Exception {
        // DB에서 파일명 제거
        uploadService.deleteAttach(fileName);
        // 첨부파일 갯수 갱신
        uploadService.updateAttachCnt(cno);
        // 파일 삭제
        UploadFileUtils.removeFile(uploadPath, fileName);
        return new ResponseEntity<>("DELETED", HttpStatus.OK);
    }
